...
from utils import createFlowDataset
...
def create_intensity_plot_hidden():
    ...
    with plt.rc_context({**bundles.neurips2024(), **axes.lines()}):
        ...
        plt.savefig("../fig/additional/intensity_hidden.pdf")
...
def create_intensity_plot_block2():
    ...
    with plt.rc_context({**bundles.neurips2024(), **axes.lines()}):
        ...
        plt.savefig("../fig/additional/intensity_block2.pdf")
...
def intensity_comparison_plot():
    ...
    flow_datasets = createFlowDataset(categories, '../FNN_sampling/flowstims', mydirs, (800, 600), (144, 256),
    ...
    plt.tight_layout()
    plt.savefig('../fig/additional/intensity_comparison_microns.pdf', dpi=300)
    plt.show()
    ...
    for idx, prefix in enumerate(prefixes):
        ...
        if (os.path.exists(cache_file_inc_mean_old) and os.path.exists(cache_file_dec_mean_old) and
            ...
            for run in range(n_runs):
                print(run)
                clf = LogisticRegression(max_iter=100, solver='lbfgs', random_state=42+run)
...
def plot_comparison_hist_and_scatter(
    pink_osi, pink_dsi, flow_osi, flow_dsi, save_path="../fig/additional/osi_dsi_comparison.pdf"
):
...
def create_osi_dsi_plot():
    ...
    plot_comparison_hist_and_scatter(pink_osi, pink_dsi, flow_osi, flow_dsi, save_path="../fig/additional/osi_dsi_comparison.pdf")
