"""
Utils for tubularity experiments. 
Also contains some toy data experiments when running this file directly.

Calculates S_tight, capturing the tightness of a trajectory bundle around its centerline, 
and S_cross, counting the amount of transverse crossings between trajectories.
"""
...
def plot_scenario_overview_proper(curves_raw, curves_processed, results, labels, title, layer=None):
    ...
    #with plt.rc_context({**bundles.neurips2024(), **axes.lines()}):
        ...
        plt.yticks([])
        if layer:
            plt.savefig(f"../fig/tubularity/Tubes_{layer}.png")
        else:
